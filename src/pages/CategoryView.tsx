  ...
  if (!category) {
    return (
      <div className="py-20 text-center">
        <h2 className="text-2xl font-bold text-stone-800 mb-4">Categoria não encontrada</h2>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-4">
      <nav className="flex items-center text-sm font-medium text-stone-500 mb-8 overflow-x-auto whitespace-nowrap pb-2">
        <Link to="/" className="group flex items-center gap-1 transition-all">
          <HomeIcon size={16} className="group-hover:text-red-500 transition-colors" />
      ...
      <div className="flex items-end justify-between mb-8 border-b border-rose-100 pb-6 w-full">
        <div className="w-full">
          <h1 className="text-3xl font-extrabold text-stone-800 mb-2">{category.name}</h1>
